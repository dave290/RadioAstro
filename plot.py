...
def do_it(frequency,intensity,parameter,startline,endline,coordinate):
    gaussAA=[0];gaussBB=[0];gaussCC=[0];gaussTT=[0]
    #calculate gaussian function
    totallines=endline-startline+1
    for j in range(totallines):
        exponentA=(-1*(frequency[j]-parameter[0])**2)/(2*(parameter[2])**2)
        gaussA=(parameter[1])*(2.718**exponentA)
        gaussAA.append(gaussA)
        exponentB=(-1*(frequency[j]-parameter[3])**2)/(2*(parameter[5])**2)
        gaussB=(parameter[4])*(2.718**exponentB)         
        gaussBB.append(gaussB)
        exponentC=(-1*(frequency[j]-parameter[6])**2)/(2*(parameter[8])**2)
    ...
    ax.scatter(frequency,intensity, marker=".", c="red")
    ax.scatter(frequency,gaussTTT, marker=".", c="blue")
    plt.show()
