...
#Note that suffixes A,B and C below do NOT refer to peaks A,B,C as
#defined in the original parameter file in fit.py
#Instead, A is "Active".  It is the peak being fit,
#while B and C are static

def center(frequency,intensity,temp_param,startline,endline,steplist):
    ...
    for i in range(max_passes):
        ...
        for j in range(totallines):
            ...
            exponentA=(-1*(frequency[j]-centerA)**2)/(2*sigmaA**2)
            gaussA=coefficientA*(2.718**exponentA)
            gaussAA.append(gaussA)
            exponentB=(-1*(frequency[j]-centerB)**2)/(2*sigmaB**2)
            gaussB=coefficientB*(2.718**exponentB)
            gaussBB.append(gaussB)
            exponentC=(-1*(frequency[j]-centerC)**2)/(2*sigmaC**2)
...
def coef(frequency,intensity,temp_param,startline,endline,steplist):
    totallines=endline-startline+1
    sumtermsarray=[0];sumterms=[0]
    centerA=temp_param[0];coefficientA=temp_param[1];sigmaA=temp_param[2]
    centerB=temp_param[3];coefficientB=temp_param[4];sigmaB=temp_param[5]
...
def sigma(frequency,intensity,temp_param,startline,endline,steplist):
    totallines=endline-startline+1
    sumtermsarray=[0];sumterms=[0]
    centerA=temp_param[0];coefficientA=temp_param[1];sigmaA=temp_param[2]
    centerB=temp_param[3];coefficientB=temp_param[4];sigmaB=temp_param[5]
